            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="text-end">
                            <a href="{{ route('taskcreate') }}" class="btn btn-primary">Add {{ $title }}</a>
        ...
        $(document).ready(function() {
            ...
            function categorytable(status=""){
                var table =  $("#subevent_list").DataTable({
                    ...
                    serverSide: true,
                    ajax: {
                    url: "{{ route('gettasklistdata') }}", // Server-side URL
                    data: function (d) {
            ...
            $(document).ready(function () {
                $(document).on("change", ".toggle-switch", function () {
                    ...
                    $.ajax({
                        url: "{{ route('taskstatuschange') }}", // Change to your backend script
                        type: "POST",
            ...
            function deleted(items) {
                swal.fire({
                    title: 'Are you sure?',
                    text: "Are you sure you want to Delete Task List?",
                    type: 'warning',
                    ...
                    if (result.value) {
                        ...
                        $.ajax({
                            url: `{{ url('taskdelete') }}/${items}`, // Dynamically include 'items' in the URL
