            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="text-end">
                            <a href="{{ route('prefixcreate') }}" class="btn btn-primary">Add {{ $title }}</a>
        ...
        $(document).ready(function() {
            ...
            function categorytable(status=""){
                var table =  $("#subevent_list").DataTable({
                    ...
                    serverSide: true,
                    ajax: {
                    url: "{{ route('getprefixlistdata') }}", // Server-side URL
                    data: function (d) {
            ...
            $(document).ready(function () {
                $(document).on("change", ".toggle-switch", function () {
                    ...
                    $.ajax({
                        url: "{{ route('prefixstatuschange') }}", // Change to your backend script
                        type: "POST",
            ...
            function deleted(items) {
                swal.fire({
                    ...
                    if (result.value) {
                        ...
                        $.ajax({
                            url: `{{ url('prefixdelete') }}/${items}`, // Dynamically include 'items' in the URL
            ...
            function deletedchecked() {
                    ...
                    swal.fire("! Opps ", "Please select Prefix to delete", "error");
                    }
            }
         ...
         function deletedcheckeditem(items) {
             swal.fire({
                 title: 'Are you sure?',
                 text: "Are you sure you want to Delete Prefix?",
                 type: 'warning',
                 showCancelButton: true,
                 confirmButtonText: 'Yes'
             }).then(function(result) {
                 if (result.value) {
                    $.ajaxSetup({
                     ...
                     $.ajax({
                         url: `{{ route('deleteselectedprefix') }}`,
                         type: 'POST',
                         data:{
                                items: items
                            },
