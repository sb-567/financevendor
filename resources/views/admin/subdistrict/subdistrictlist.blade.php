            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="text-end">
                            <a href="{{ route('subdistrictcreate') }}" class="btn btn-primary">Add {{ $title }}</a>
        ...
        $(document).ready(function() {
            ...
            function categorytable(status=""){
                var table =  $("#subevent_list").DataTable({
                    ...
                    serverSide: true,
                    ajax: {
                    url: "{{ route('getsubdistrictlistdata') }}", // Server-side URL
                    data: function (d) {
            ...
            $(document).ready(function () {
                $(document).on("change", ".toggle-switch", function () {
                    ...
                    $.ajax({
                        url: "{{ route('subdistrictstatuschange') }}", // Change to your backend script
                        type: "POST",
            ...
            function deleted(items) {
                swal.fire({
                    title: 'Are you sure?',
                    text: "Are you sure you want to Delete Sub District List?",
                    type: 'warning',
                    ...
                    if (result.value) {
                        ...
                        $.ajax({
                            url: `{{ url('subdistrictdelete') }}/${items}`, // Dynamically include 'items' in the URL
                            ...
                            success: function(data) {
                                swal.fire({
                                    type: 'success',
                                    title: 'Sub District Deleted Successfully',
                                });
                            },
                            complete: function() {
                                swal.hideLoading();
            ...
            function deletedchecked() {
                    ...
                    swal.fire("! Opps ", "Please check Sub District to delete", "error");
                    }
            }
         ...
         function deletedcheckeditem(items) {
             swal.fire({
                 title: 'Are you sure?',
                 text: "Are you sure you want to Delete Sub District?",
                 type: 'warning',
                 showCancelButton: true,
                 confirmButtonText: 'Yes'
             }).then(function(result) {
                 if (result.value) {
                    $.ajaxSetup({
                     ...
                     $.ajax({
                         url: `{{ route('deleteselectedsubdistrict') }}`,
                         type: 'POST',
                         data:{
                                items: items
                            },
